...
pub fn construct(key_string: &str) -> Box<dyn Filter> {
    let n: u8 = key_string.parse().expect("Failed to parse key string to u8");
...
impl Filter for Mult {
    ...
    fn apply(&self, image_buffer: &mut RgbImageBuffer) {
        ...
        for x in 0..width  {
            ...
            let new_pixel = mult_to_pixel(pixel, self.n);
...
fn mult_to_pixel(pixel: &Rgb<u8>, n: u8) -> Rgb<u8> {
    let (r, g, b) = (pixel[0], pixel[1], pixel[2]);

    let newr = r.saturating_mul(n);
    let newg = g.saturating_mul(n);
    let newb = b.saturating_mul(n);
